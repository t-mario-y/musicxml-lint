  ...
  Array.from(measureArray)
    ...
    .forEach(measure => {
      ...
      noteArray.forEach(note => {
        ...
        if (
          note.querySelectorAll("lyric").length === 0 &&
...
class lintResultObj {
  constructor(_violartionKind, _partId, _partName, _measurePlace) {
    ...
    this.partId = _partId;
    this.partName = _partName.normalize("NFKC");
    this.measurePlace = _measurePlace;
  }
}

const convertPartIdToPartName = (dom, partId) => {
  return dom.querySelector(`score-part#${partId}`).querySelector(`part-name`)
    .textContent;
};
